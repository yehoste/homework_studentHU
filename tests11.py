...
# then run pytest                            #
# shalom blau                                #
# shalom.blau@mail.huji.ac.il                #
##############################################

from tree import Node, Diagnoser, Record, build_tree, optimal_tree, parse_data
...
root13 = Node(1)
root13.yes_child = Node(2)
root13.yes_child.yes_child = Node('a')
root13.yes_child.no_child = Node('b')
root13.no_child = Node(3)
root13.no_child.yes_child = Node(4)
root13.no_child.yes_child.yes_child = Node(None)
root13.no_child.yes_child.no_child = Node(None)
root13.no_child.no_child = Node(5)
root13.no_child.no_child.yes_child = Node(None)
root13.no_child.no_child.no_child = Node(3)
root13.no_child.no_child.no_child.yes_child = Node(None)
root13.no_child.no_child.no_child.no_child = Node(4)
root13.no_child.no_child.no_child.no_child.yes_child = Node(None)
root13.no_child.no_child.no_child.no_child.no_child = Node(2)
root13.no_child.no_child.no_child.no_child.no_child.yes_child = Node('a')
root13.no_child.no_child.no_child.no_child.no_child.no_child = Node('b')
...
def print_in_order(diagno: Diagnoser):
    ...
    def print_in_order_helper(cur_node: Node):
        if cur_node is None:
            return
        if cur_node.yes_child:
            print_in_order_helper(cur_node.yes_child)
        printable[0] += str(cur_node.data) + ' '
        if cur_node.no_child:
            print_in_order_helper(cur_node.no_child)

    print_in_order_helper(diagno.root)
    return printable[0][:-1]


def print_neatly(diagno: Diagnoser):
    printable = [""]

    def print_neatly_helper(cur_node: Node, depth: int):
        if cur_node is None:
            return

        if cur_node.yes_child:
            printable[0] += str(cur_node.data) + '?\n'
            printable[0] += '\t' * (depth + 1) + 'yes: '
            print_neatly_helper(cur_node.yes_child, depth + 1)
            printable[0] += '\t' * (depth + 1) + 'no: '
            print_neatly_helper(cur_node.no_child, depth + 1)
        else:
            printable[0] += str(cur_node.data) + '!\n'

    print_neatly_helper(diagno.root, 0)
    return printable[0]
...
def test_build_tree1():
    ...
    assert "meningitis" == tree1.yes_child.yes_child.data
    assert "influenza" == tree1.yes_child.no_child.data
    assert "cold" == tree1.no_child.yes_child.data
    assert "healthy" == tree1.no_child.no_child.data
...
def test_build_tree2():
    ...
    assert "influenza" == tree2.yes_child.yes_child.data
    assert "cold" == tree2.yes_child.no_child.data
    assert "strep" == tree2.no_child.yes_child.data
    assert "healthy" == tree2.no_child.no_child.data
...
def test_build_tree3():
    ...
    assert "influenza" == tree3.yes_child.yes_child.data
    assert "meningitis" == tree3.yes_child.no_child.data
    assert "cold" == tree3.no_child.yes_child.data
    assert "healthy" == tree3.no_child.no_child.data
...
def test_build_tree4():
    ...
    assert "influenza" == tree4.yes_child.yes_child.data
    assert "strep" == tree4.yes_child.no_child.data
    assert "cold" == tree4.no_child.yes_child.data
    assert "healthy" == tree4.no_child.no_child.data
...
def test_build_tree_empty_records():
    ...
    assert tree4.yes_child.yes_child.data is None
    assert tree4.yes_child.no_child.data is None
    assert tree4.no_child.yes_child.data is None
    assert tree4.no_child.no_child.data is None
...
def test_build_tree_empty_symptoms():
    ...
    assert tree1.yes_child is None and tree1.no_child is None

    assert tree2.data == "indigestion"
    assert tree2.yes_child is None and tree2.no_child is None
...
def test_optimal_tree1():
    ...
    assert "cough" == tree.yes_child.data or "fever" == tree.yes_child.data
...
def test_optimal_tree3():
    ...
    assert "influenza" == tree.yes_child.yes_child.data
    assert "meningitis" == tree.yes_child.no_child.data
    assert "cold" == tree.no_child.yes_child.data
    assert "healthy" == tree.no_child.no_child.data
...
def test_optimal_tree_errors():
    ...
    try:
        ...
        assert str(error5) != '', "you should add an error message to the error you raising (the error case checked is a symptom that appear twice)"
        for e in errors:
            assert str(error5) != e, "you should add a *different* error message to the error you raising (the error case checked is a symptom that appear twice)"
        errors[3] = str(error5)
...
def test_optimal_tree_empty_symptoms():
    ...
    assert tree3.yes_child is None and tree1.no_child is None

    assert tree4.data == "indigestion"
    assert tree4.yes_child is None and tree2.no_child is None
...
def test_minimize_built_tree_optimal_tree_calculate_success_rate():
    def check(n: Node):
        def helper(a, b):
            if not a and not b:
                return True
            ...
            if a.data != b.data:
                return False
            if helper(a.yes_child, b.yes_child):
                return helper(a.no_child, b.no_child)
            return False

        if n.yes_child is None:
            return False
        return helper(n.yes_child, n.no_child)
    ...
    def check_for_nones(cur_root: Node):
        ...
        if cur_root.data is None:
            return True
        if cur_root.yes_child:
            is_there_none_in_pos = check_for_nones(cur_root.yes_child)
            is_there_none_in_neg = check_for_nones(cur_root.no_child)
    ...
    list_of_rates = [rates_optimal[0], rates_optimal[1], rates_optimal[2], rates_built_tree]
    for i in range(len(list_of_rates)):
        rates = list_of_rates[i]
        for ind, file in enumerate(files):
            symptoms = []
            records = parse_data(file)
            for record in records:
                for s in record.symptoms:
                    if s not in symptoms:
                        symptoms.append(s)
            if i < len(list_of_rates) - 1:
                diagno = optimal_tree(records, list(symptoms), i)
            else:
                symptoms += [symptoms[0]]
                diagno = build_tree(records, list(symptoms))
            rate_before_minimizing = diagno.calculate_success_rate(records)
            diagno.minimize(False)
            assert check(diagno.root) is False, f"\nyour minimize(False) function did not remove all duplications!\nfile name: {file},\nthe tree was built using the optimal_tree function with a depth of {i}" if i < 3 else f"\nyour minimize(False) function did not remove all duplications!\nfile name: {file},\nthe tree was built using the built_tree function\n the tree currently looks like this:\n{print_neatly(diagno)}"
            rate_after_removing_duplications = diagno.calculate_success_rate(records)
            diagno.minimize(True)
            assert check_for_nones(diagno.root) is False, f"\nyour minimize(True) did not remove all nones!\nfile name: {file},\nthe tree was built using the optimal_tree function with a depth of {i}" if i < 3 else f"\nyour minimize(True) did not remove all nones!\nfile name: {file},\nthe tree was built using the built_tree function\n the tree currently looks like this:\n{print_neatly(diagno)}"
            rate_after_removing_empties = diagno.calculate_success_rate(records)
            assert rate_before_minimizing == rate_after_removing_duplications, f"\nyour minimize(False) function changes the rate of success of the diagnoser, it should not do that!\nbefore: {rate_before_minimizing}, after: {rate_after_removing_duplications}\nfile name: {file},\nthe tree was built using the optimal_tree function with a depth of {i}\n the tree currently looks like this:\n{print_neatly(diagno)}" if i < 3 else f"\nyour minimize(False) function changes the rate of success of the diagnoser, it should not do that!\nbefore: {rate_before_minimizing}\nafter: {rate_after_removing_duplications}\nfile name: {file}\nthe tree was built using the built_tree function\n the tree currently looks like this:\n{print_neatly(diagno)}"
